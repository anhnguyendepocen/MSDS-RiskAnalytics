...
So divide a year into 25,000 sub-periods (think of these sub-periods roughly as minutes) instead, and compare the new mu (average log return per minute) with the new $m-\frac{s^2}{2}$ (m is now the average net return per minute). 
...
new.mu <- annual.ret / trading.periods
new.sigma <- annual.vol / sqrt(trading.periods)

min.logRet <- rnorm( min_from_days(250), new.mu, new.sigma)
...
+ New m = $`r format(round(new.m, 5 ), scientific=F)`$
...
ggplot(data.table(y = logPrice)[, x := .I], aes( x, y)) +
  ...
  labs(title = "Log Stock Price After t", x = "Time", y = "Log Price") +
  theme(plot.title = element_text(hjust = 0.5))
