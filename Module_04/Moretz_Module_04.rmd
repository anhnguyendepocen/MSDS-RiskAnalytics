...
Here we are fitting the copulas to the uniform-transformed data.
...
However, I think method one would be a more robust estimte due to the estimates coming from the fitted t-distribution.
...
ll <- 2 * pstd( - sqrt( (nu+1) * ( 1-rho) / (1+rho) ), nu + 1)

```

$\lambda_{\ell} = `r format(round(ll, 6), scientific = F)`$