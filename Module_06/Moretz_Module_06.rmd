...
returnBlMon <- SP500$r500

p1 <- ggplot(data.table(ret = returnBlMon), aes(ret, y = ..density..)) +
   geom_histogram(aes(fill = ..count..), bins = 50) +
   geom_density(aes(y = ..density..), col = "darkred", lwd = 1)

x <- SP500$r500[(1804 - 2*253+1 ): 1804]

p2 <- ggplot(data.table(ret = c(x, returnBlMon))[, x := .I], aes(x, ret)) +
   geom_line() +
   labs(title = "SP500 Returns")
...
summary(fit <- ugarchfit(data = x, spec = spec))
