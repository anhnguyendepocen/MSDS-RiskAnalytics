...
$w_1 = \frac{S_1}{S_1 + S_2}$, $w_2 = 1 - w_1$
...
VaR_N = function( alpha, mu, v, S ) {
   - S * ( mu + qnorm(alpha) * sqrt(v)  )
}
...
var1d <- VaR_N(0.1, port.ret, port.var, capital)
...
port.var <- 5 * sum( stock.weight**2 * diag(stock.cov) )

var5d <- VaR_N(0.1, port.ret, port.var, capital)
